(function() {
    ...
    function Product($state, routerHelper,logger,common,config,ProductService,beerboardservice,beerboard,_,$sce,$rootScope,$scope,$timeout,$document,$window) {    
        ...
        $rootScope.initialBeerList[bb_config.locationId + '/' + bb_config.templateId] = "";
        $rootScope.currentBeerList=[];
        $rootScope.locationSpecils =false;        
        $rootScope.isBeerDataChanged = [];         
        $rootScope.isBeerDataChanged[bb_config.locationId + '/' + bb_config.templateId] = false;
        ...
        function getBeers(){          
             ...
             return beerboardservice.getMenus({locationId : bb_config.locationId,bar_id : bb_config.bar_id,product_type : bb_config.product_type}).then(function(data) {   
                ...
                $rootScope.initialBeerList[bb_config.locationId + '/' + bb_config.templateId] = JSON.stringify(data);
                     
                 vm.locationSequence = data.locationSequence;
                 vm.draftBeers = [];
                 vm.draftBeers =  data.draftBeers;
                                 
                 vm.marketingBeerListData = _.filter(vm.draftBeers, function(item){ return item.draftBeer.local>0 || item.draftBeer.featured>0 || item.draftBeer.cask>0 });
                    _.each(vm.marketingBeerListData,function(res){                        
                        vm.marketingBeerList.push(res.draftBeer);
                    });
                 if(data.onDeck.length>0)
                 {
                    _.each(data.onDeck,function(res){                        
                        vm.marketingBeerList.push(res.product);
                    });
        ...
        //    $scope.$on('$destroy', function(){$timeout.cancel(vm.timerForChanges);});
            if($rootScope.isBeerDataChanged[bb_config.locationId + '/' + bb_config.templateId] == true)
            {
                    $timeout(function() {
        ...
        //
        //*************************************************************************************         
         $scope.showMarketingSlide = function() { debugger
            if($rootScope.isIframe){
            $rootScope.showMarketingBeerItem=false;   
            }
            else{
            ...
            $rootScope.marketingBeerItem = asset;            
            if(asset.cask==undefined && asset.featured==undefined && asset.local==undefined)
            {
                $rootScope.onDeckBeerAvailable = true;
            }
            else
            {
                $rootScope.onDeckBeerAvailable = false;
