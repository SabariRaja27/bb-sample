        ...
        function setCache(){
            ...
            CacheFactory('menusCache', { 
                ...
                onExpire: function (key, value) { //Added by Kamal, when maxAge completes calling API to get updated values
                    ...
                    console.log('key= ' + key);
                    //checking whether the kay contains our actual locationId
                    //$rootScope.keyDuplicationCheck = key.includes('locationId=' + bb_config.locationId);  
                    $rootScope.keyDuplicationCheck = key.indexOf('locationId=' + bb_config.locationId)>0;                  
                    //If the key contains actual locationId,then only the onExpire process will starts execute to append the new data.
                    if($rootScope.keyDuplicationCheck){
                        var deferred = $q.defer(); //promise
                        $http({
                        method: 'GET',
                        url: key,
                       ...
                       // console.log(JSON.stringify(data));
                            ...
                            $rootScope.comparedBeerData = _.isEqual($rootScope.initialBeerList[bb_config.locationId + '/' + bb_config.templateId], $rootScope.currentBeerList );
                            /*if there is any data changes happens ,the changed data will be used*/
                                ...
                                $rootScope.isBeerDataChanged[bb_config.locationId + '/' + bb_config.templateId] = true;
